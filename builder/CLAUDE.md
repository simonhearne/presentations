...
**Iframe frontmatter** — a fenced ` ```iframe ` block embeds an external page on the slide (kv-list shape; `url` required). The frame is emitted with **`data-src`, never `src`**: [script/iframe.js](script/iframe.js) sends a `no-cors` `fetch` probe at the host on load and only assigns `src` once something answers, adding `.iframe-live` to the `<section>` and recording `data-iframe-state="pending|live|offline"`. An iframe gives no usable failure signal of its own — `onerror` doesn't fire for a refused connection or a cross-origin block — so the probe is the only way to know. The probe re-runs on `slide:enter` for slides that aren't live yet, so a demo started after the deck was opened is picked up by navigating away and back. Per-embed keys: `probe` (alternate URL, or `false` to skip and always load) and `probe-timeout` (ms, default 1500).
