...
**Iframe frontmatter** — a fenced ` ```iframe ` block embeds an external page on the slide (kv-list shape; `url` required). The frame is emitted with **`data-src`, never `src`**: [script/iframe.js](script/iframe.js) sends a `no-cors` `fetch` probe at the host on load and only assigns `src` once something answers, adding `.iframe-live` to the `<section>` and recording `data-iframe-state="pending|live|offline"`. An iframe gives no usable failure signal of its own — `onerror` doesn't fire for a refused connection or a cross-origin block — so the probe is the only way to know. The probe re-runs on `slide:enter` for slides that aren't live yet, so a demo started after the deck was opened is picked up by navigating away and back. Per-embed keys: `probe` (alternate URL, or `false` to skip and always load) and `probe-timeout` (ms, default 1500). The probe is also gated on the deck's own origin: a loopback/private/`.local` target is probed only when the deck is served locally (`file://`, `localhost`, private IP) or the URL carries `?simon=true`, otherwise it goes straight to `offline`. Chrome's Local Network Access permission prompts on any public-origin request to a local address, so the hosted deck would otherwise ask every viewer for a host only the presenter can reach.

A companion ` ```iframe-fallback ` block on the same slide holds markdown shown in the frame's place; `renderIframeFallback` substitutes `IFRAME_FALLBACK_DEFAULT` when it's absent, so an unreachable host never yields a blank slide. An optional `still` key on the `iframe` block (plus `still-alt`) names a screenshot of the demo: `copyIframeStills` copies it into `dist/` like `copyAuthorPhotos` does, and it renders as `<img class="iframe-still">` inside the fallback, sharing the frame's exact geometry so the fallback slide reads as the demo slide. An optional `fallback-offset` (any CSS length) is emitted as `--iframe-fallback-offset` inline on the fallback wrapper, which `css/layouts.css` reads as `margin-top`. All three keys are filtered out of the pass-through `data-*` attrs. Because the frame has no `src` until JS proves the host is up, anything that doesn't run the probe — print, PDF export, a saved copy, the hosted deck at talks.simonhearne.com — renders the fallback by default rather than a broken frame. `css/print.css` forces the fallback unconditionally. Keep the fallback in the *same* `<section>` as the frame: `script/deck.js` snapshots `.slide` elements once at load and page numbers are baked in at build time, so a separate fallback section would desync both.

**Fragments** — Pandoc-style `{.fragment}` markers on block elements (`<li>`, `<p>`, `<h1>`–`<h6>`, `<blockquote>`) and inline `[text]{.fragment}` spans drive incremental reveals. `bin/build.js` runs `applyFragmentAttrs` on the rendered HTML to strip markers and merge classes; `script/fragments.js` registers a capture-phase keydown listener that reveals/hides on the active slide. `script/deck.js` dispatches a `slide:enter` CustomEvent on `document` with `detail: { direction, index, slide }` whenever the active slide changes, where `direction` is `'forward' | 'backward' | 'jump'`. Fragments listen and reset hidden on `forward`/`jump`, pre-reveal on `backward`. `stepsOn` groups fragments into steps by `data-fragment-index` (unindexed fragments take the previous index + 1, so plain decks still step in document order) and each keypress reveals or hides a whole step; `markCurrent` keeps `is-current` on the newest fully revealed step so deck CSS can highlight it (see `.phases` in `css/layouts.css`). The presenter cue lives in `script/deck.js` instead, because a slide can stage advances through more than one runtime: `window.deckSteps.register(counter)` takes a function returning `{ total, remaining }` steps for a slide, `window.deckSteps.refresh(slide)` recomputes, and `fragments-done` goes on the `<section>` only when every registered counter reports `total > 0` and nothing remaining. `script/fragments.js` registers its fragment steps and `script/vega.js` its `signal-*` step lists (a list of n values is n - 1 advances, since the first is seeded on embed), so a slide whose last advance is a chart stage lights the dot at the right moment; `css/deck.css` draws that dot centred on the footer rule as a cue that the next advance changes slide. Anything else that consumes arrow keys on a slide (a three.js module) can join by registering the same shape. Capture-phase listener registration order in the template is `fragments → vega → three`, so fragments consume keypresses first — which is why a chart's stages ran after every fragment on its slide, whatever the author wanted. `fragment-index` on a ` ```vega ` block fixes the order: it lands as `data-fragment-index` on the chart `<div>`, `script/vega.js` exposes `window.deckCharts.index(slide)` (the earliest slot any chart on the slide claims, `Infinity` when none does) and `window.deckCharts.pending(slide, direction)` (whether a stage is still owed that way), and `stepFragments` declines to consume the keypress when the step in hand is at or after that slot — handing it to vega's listener, next in the chain. A chart's whole stage run occupies one slot, ties go to the chart, and a chart without the key answers `Infinity` so every existing slide keeps stepping exactly as it did. Backward is the mirror: a revealed step below the chart's slot hands the key back so the stages rewind first. `stepsOn` therefore returns `{ index, els }` per step rather than a bare element array. `script/fragments.js` also reveals every fragment in the deck for print and restores the live step afterwards, listening on both `beforeprint`/`afterprint` (the File > Print route) and the `matchMedia('print')` change (headless `page.pdf()`, Safari); `css/print.css` forces `.fragment` visible as the no-JS fallback, but only the JS path adds `is-revealed`, which is what deck CSS keyed on that class needs.
...
- CSS: follow [docs/styleguide.md](docs/styleguide.md) before adding any rule. Compose the shared components (`.card`, `.card-grid`, `.pill`, `.stat-grid`) and `--zilliz-*` tokens first; deck one-offs go at the bottom of `css/layouts.css` under a `/* ── Deck: talks/<slug> ── */` banner and are deleted when the deck retires.
